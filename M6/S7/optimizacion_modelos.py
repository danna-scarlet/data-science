...
# 1. Carga de datos
reg = pd.read_csv('C:/Users/danna/OneDrive/Documentos/GitHub/data-science/M6/S7/datos_regresion 2.csv')
clas = pd.read_csv('C:/Users/danna/OneDrive/Documentos/GitHub/data-science/M6/S7/datos_clasificacion 1.csv')
...
Xc_train_smote, yc_train_smote = smote.fit_resample(Xc_train, yc_train)
clas_model_smote = LogisticRegression()
clas_model_smote.fit(Xc_train_smote, yc_train_smote)
yc_pred_smote = clas_model_smote.predict(Xc_test)
print(f"F1-score con SMOTE: {f1_score(yc_test, yc_pred_smote):.2f}")
print(f"Matriz de confusión (SMOTE):\n{confusion_matrix(yc_test, yc_pred_smote)}")

# SMOTEENN (Combinación de sobremuestreo y submuestreo)
smoteenn = SMOTEENN(random_state=42)
Xc_train_smoteenn, yc_train_smoteenn = smoteenn.fit_resample(Xc_train, yc_train)
clas_model_smoteenn = LogisticRegression()
clas_model_smoteenn.fit(Xc_train_smoteenn, yc_train_smoteenn)
yc_pred_smoteenn = clas_model_smoteenn.predict(Xc_test)
print(f"F1-score con SMOTEENN: {f1_score(yc_test, yc_pred_smoteenn):.2f}")
print(f"Matriz de confusión (SMOTEENN):\n{confusion_matrix(yc_test, yc_pred_smoteenn)}")

# Curvas ROC para todos los modelos balanceados
...
fpr_smote, tpr_smote, _ = roc_curve(yc_test, clas_model_smote.predict_proba(Xc_test)[:, 1])
fpr_smoteenn, tpr_smoteenn, _ = roc_curve(yc_test, clas_model_smoteenn.predict_proba(Xc_test)[:, 1])
auc_ros = auc(fpr_ros, tpr_ros)
auc_rus = auc(fpr_rus, tpr_rus)
auc_smote = auc(fpr_smote, tpr_smote)
auc_smoteenn = auc(fpr_smoteenn, tpr_smoteenn)

plt.figure(figsize=(10, 7))
plt.plot(fpr_ros, tpr_ros, label=f'ROC ROS (AUC={auc_ros:.2f})')
plt.plot(fpr_rus, tpr_rus, label=f'ROC RUS (AUC={auc_rus:.2f})')
plt.plot(fpr_smote, tpr_smote, label=f'ROC SMOTE (AUC={auc_smote:.2f})')
plt.plot(fpr_smoteenn, tpr_smoteenn, label=f'ROC SMOTEENN (AUC={auc_smoteenn:.2f})')
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('Tasa de Falsos Positivos')
plt.ylabel('Tasa de Verdaderos Positivos')
